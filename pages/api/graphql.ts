...
const getTaskById = async (id: number, db: mysql.ServerlessMysql) => {
  const tasks = await db.query<TaskDbQueryResult>("SELECT id, title, task_status FROM task WHERE id = ?", [id]);
...
const resolvers: Resolvers<ApolloContext> = {
  ...
  Mutation: {
    ...
    async updateTask(parent, args, context) {
      ...
      await context.db.query(`UPDATE task SET ${columns.join(",")} where id = ?`, sqlParams);
    ...
    async deleteTask(parent, args, context) {
      ...
      await context.db.query("DELETE FROM task WHERE id = ?", [args.id]);
